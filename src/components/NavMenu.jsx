...
export function NavMenu({ route }) {
  ...
  return (
    <nav className="mb-8 flex items-center gap-2 border-b border-div2-border pb-4">
      <span className="hidden sm:inline text-div2-orange text-[11px] font-mono uppercase tracking-[0.28em] mr-2">
        ISAC //
      </span>
      {LINKS.map(({ route: linkRoute, href, labelKey }) => {
        ...
        return (
          <a
            ...
            className={`clip-panel-sm px-4 py-2 text-xs font-mono font-bold uppercase tracking-widest border transition-all duration-150 ${
              active
